...
//http://localhost/pms/jsp/project/assets/info-icon_24_24.png
 ...
 // 간트 데이터 얻기
 function saveGanttData(infoData, isSaveAs, location) {
...
function saveAsExcel( timeUnit ) {
	...
	var exportProps = {
		fileName : prjInfo.name, // 파일 명 지정  
		sheetName : prjInfo.name, // 엑셀 시트명 지정
...
function moveProjectOutput(){
    ...
    if(oid != '' && oid.length > 0){
    	var url =  "<c:url value='/jsp/project/project-out.jsp'/>?oid=" + oid;
    	axDialog.confirm({
    	    msg: LANG("ax.script.ks.16")
    	}, function () {
    	    if (this.key == "ok") {
    	    	saveGanttData(getInitData(), false, url);
    	    } else{
...
function moveCalendar(){
    ...
    if(oid != '' && oid.length > 0){
    	...
    	var url =  "<c:url value='/jsp/project/project-calendar.jsp'/>?oid=" + oid;
