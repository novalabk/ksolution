...
import com.boot.ksolution.core.db.mapper.ColumnToBeanPropertyRowMapper;
...
public abstract class KSolutionBaseService<T, ID extends Serializable> extends KSolutionFilterService<T> {
	
	protected KSolutionJPAQueryDSLRepository<T, ID> repository;
	
	public KSolutionBaseService() {
	}
	
	public KSolutionBaseService(KSolutionJPAQueryDSLRepository<T, ID> repository) {
    ...
    public <S extends T> S save(S var) {
        ...
        if (var instanceof KSolutionCrudModel) {
        	KSolutionCrudModel crudModel = (KSolutionCrudModel) var;

            if (crudModel.getDataStatus() == KSolutionTypes.DataStatus.DELETED) {
                deleted = true;
            }
        }
