package com.boot.ksolution.core.db;
...
/*prepare sql 을 파라미터를 넣어서 sql를 구성해주는 클래스 주의 index 1부터 시작 된다 */
public class QueryNormalizer {
	private static final String DATE_FORMAT = "yyyy/MM/dd HH:mm:ss.SSS";
	private Boolean dumpBooleanAsTrueFalse = false;

	/**  
	 * @param databaseType  (MYSQL,ORCLE...)
	 * @param query     (select * from ttt where id =? and name= ?)
	 * @param parameters (index 1부터 prepareStatem에서 set(1, "kkk") 처러 1부터 리스트한다. 주의
	 * @return  
	 */
	...
	private String convert(String databaseType, Object parameter) {
		...
		if (KSolutionTypes.DatabaseType.ORACLE.equals(databaseType)) {
