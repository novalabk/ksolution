...
public class AuthGroupMenu extends BaseJpaModel<AuthGroupMenu.AuthGroupMenuId>{
    ...
    @Comment(value = "조회권한")
    @ColumnPosition(3)
    private String schAh = KSolutionTypes.Used.NO.getLabel();

    @Column(name = "SAV_AH", length = 1)
    @Comment(value = "저장권한")
    @ColumnPosition(4)
    private String savAh = KSolutionTypes.Used.NO.getLabel();

    @Column(name = "EXL_AH", length = 1)
    @Comment(value = "엑셀권한")
    @ColumnPosition(5)
    private String exlAh = KSolutionTypes.Used.NO.getLabel();

    @Column(name = "DEL_AH", length = 1)
    @Comment(value = "삭제권한")
    @ColumnPosition(6)
    private String delAh = KSolutionTypes.Used.NO.getLabel();

    @Column(name = "FN1_AH", length = 1)
    @Comment(value = "기능키1권한")
    @ColumnPosition(7)
    private String fn1Ah = KSolutionTypes.Used.NO.getLabel();

    @Column(name = "FN2_AH", length = 1)
    @Comment(value = "기능키2권한")
    @ColumnPosition(8)
    private String fn2Ah = KSolutionTypes.Used.NO.getLabel();

    @Column(name = "FN3_AH", length = 1)
    @Comment(value = "기능키3권한")
    @ColumnPosition(9)
    private String fn3Ah = KSolutionTypes.Used.NO.getLabel();

    @Column(name = "FN4_AH", length = 1)
    @Comment(value = "기능키4권한")
    @ColumnPosition(10)
    private String fn4Ah = KSolutionTypes.Used.NO.getLabel();

    @Column(name = "FN5_AH", length = 1)
    @Comment(value = "기능키5권한")
    @ColumnPosition(11)
    private String fn5Ah = KSolutionTypes.Used.NO.getLabel();
	
    @Transient
    private KSolutionTypes.Used useYn;
    
    
    public void updateAuthorization(AuthGroupMenu authGroupMenu) {     //여러 구룹을 가졌을때 머지 하는 것임
        this.schAh = positive(this.schAh, authGroupMenu.getSchAh());  //기존이 Y이면 Y 새로운것이 Y이면 Y 
    ...
    public void updateAuthorization(Program program) {
        this.schAh = negative(this.schAh, program.getSchAh()); //구륩이 Y이면 교체 가능 
